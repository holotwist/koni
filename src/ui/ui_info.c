...
void draw_info_panel(int y, int x, int h, int w) {
    ...
    if (max_lines < 8) {
        ...
        mvprintw(line++, x + 2, "Sample rate : %-9u", ui_cache.fmt.sample_rate);
        mvprintw(line++, x + 2, "Bit rate    : %u kbps", ui_cache.fmt.bitrate / 1000);
        mvprintw(line++, x + 2, "Bits/sample : %-9u", ui_cache.fmt.bits_per_sample);
